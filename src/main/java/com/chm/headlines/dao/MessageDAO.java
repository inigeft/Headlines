...
public interface MessageDAO {
    ...
    int addMessage(Message message);        //返回message的id?

    @Select({"select count(id) from ", TABLE_NAME, " where has_read = 0 and to_id=#{userId} and conversation_id = #{conversationId}"})
    int getConversationUnreadCount(@Param("userId") int userID, @Param("conversationId") String conversationId);    //@Param注解勿漏
